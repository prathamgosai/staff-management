...
| 31 | _(Task 11)_ | DX — repo-wide **ESLint flat config** (root `eslint.config.mjs` + `eslint.base.mjs`, per-package configs for api/shared/web with the Next plugin), run via a tiny cross-platform `scripts/eslint.mjs` that opts ESLint 8.57 into flat mode (no `cross-env`); lenient first pass (legacy → warnings) so `pnpm lint` is green across all workspaces via the existing Turbo pipeline. API dev now hot-reloads (`nest start --watch`; old ts-node path kept as `dev:tsnode`) |
