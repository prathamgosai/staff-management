    ...
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 bg-gray-50 border-b border-gray-200">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-xs text-gray-400 font-medium">{members.length} staff</span>
        </div>
...
export default function OutletDetailPage() {
  ...
  const iconBg    = outlet ? (TYPE_ICON_BG[outlet.type] ?? TYPE_ICON_BG.other) : "bg-gray-100 text-gray-600";
  ...
  return (
    <div className="space-y-6">
      {/* Back nav */}
      <Link href="/outlets" className="inline-flex items-center gap-1.5 text-sm text-gray-500 hover:text-gray-800 transition">
        <ArrowLeft size={15} />
        Back to Outlets
      </Link>
      ...
      {outletLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-6">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${outlet.is_active ? "bg-emerald-50 text-emerald-700 ring-1 ring-emerald-200" : "bg-gray-100 text-gray-500"}`}>
              ...
              <p className="text-sm text-gray-500 mt-0.5">
                {outlet.brand_name}
                <span className="font-mono text-xs ml-2 bg-gray-100 text-gray-500 px-2 py-0.5 rounded">{outlet.code}</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-5 pt-5 border-t border-gray-100">
            {[
              { label: "Total Staff",  value: total,         color: "text-blue-600",    bg: "bg-blue-50" },
              ...
              { label: "Full Time",    value: fullTimeCount, color: "text-violet-600",  bg: "bg-violet-50" },
            ].map(stat => (
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          {!isLoading && deptKeys.length > 1 && (
            <span className="text-xs text-gray-400">{deptKeys.length} departments</span>
          )}
        </div>
        ...
        {isLoading ? (
            ...
            {Array.from({ length: 3 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl border border-gray-200 overflow-hidden animate-pulse">
                ...
                {Array.from({ length: 4 }).map((_, j) => (
                  <div key={j} className="flex items-center gap-4 px-5 py-3.5 border-b border-gray-100 last:border-0">
          ...
          <div className="bg-white rounded-2xl border border-red-200 flex flex-col items-center gap-3 py-16 text-center">
            <Users size={40} strokeWidth={1.2} className="text-red-300" />
            <div>
              <p className="font-semibold text-gray-700">Could not load staff</p>
              <p className="text-xs text-gray-400 mt-0.5">There was an error fetching staff data</p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-200 flex flex-col items-center gap-3 py-16 text-center">
            <Users size={40} strokeWidth={1.2} className="text-gray-300" />
            <div>
              <p className="font-semibold text-gray-600">No staff assigned to this outlet</p>
              <p className="text-xs text-gray-400 mt-0.5">Add staff members from the Staff page</p>
            </div>
            <Link href="/staff"
              className="mt-2 inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium px-4 py-2 rounded-lg transition">
              Go to Staff
